...
function compute_projection(D::Int)
    ...
    Λ, U = eigen(Matrix(PP))
    spectrum = Λ .|> (u -> round(u, digits = 4))
    idx = findall(abs.(spectrum .- unique(spectrum)[end]) .< 0.1)
    u = U[:, idx]
    P = sparse(u * u')
...
begin loss = loss_func; u = adam.theta
    ...
    p1 = plot(1:iter, loss_vals_adam2_l1, label="Adam2 L1, loss = $(loss_val_adam2_mle |> minimum)")
...
begin loss = l1; u = rand(Haar(2, D))
    ...
    p1 = plot(1:iter, loss_vals_adam_l1, label="Adam L1 loss = $(loss_val_adam_mle |> minimum)", title = "Optimize unitary on L1 norm")
    p2 = plot(1:iter, loss_val_adam_mle, label="Adam MLE")
    plot(p1, p2, layout=(2,1))
end
...
begin loss = loss_func; u = rand(Haar(2, D))
    ...
    p1 = plot(1:iter, loss_vals_adam_l1, label="Adam L1 loss = $(loss_val_adam_mle |> minimum)", title = "Optimize unitary on minimum eigen value")
    p2 = plot(1:iter, loss_val_adam_mle, label="Adam MLE")
    plot(p1, p2, layout=(2,1))
end
