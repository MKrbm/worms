...
def convert_spherical(input, digits= None):
    ...
    if(input.ndim==1):
        ...
        for i in range(0, len(input)-2):
            convert.append(round(acos(input[i]/r),digits))
            r=math.sqrt(r*r - input[i]*input[i])
        if(input[-2]>=0):
            convert.append(round(acos(input[-2]/r),digits))
        else:
            convert.append(round(2*math.pi - acos(input[-2]/r),digits))
        result = convert
        ...
        for element in range(0, len(input)):
            ...
            for i in range (0 ,len(input[element])-2):
                convert.append(round(acos(input[element][i] / r),digits))
                r = math.sqrt(r*r - input[element][i]*input[element][i])
            if(input[element][-2] >= 0):
                convert.append(acos(input[element][-2]/r))
            else:
                convert.append(2*math.pi - acos(input[element][-2] /r))
