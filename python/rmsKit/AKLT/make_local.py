...
models = ["original", "optm1", "optm2", "optm2_nt", "optm3"]
...
if lat == "original":
    ...
    for _ in range(M):
        ...
        cg = RiemanNonTransUnitaryCG(
            [(models[i], models[(i + 1) % L]) for i in range(L)], [loss] * L, pout=False, lr=0.01
        )
        ...
        if ret.fun < best_fun:
            ...
            best_model = ret.model
    LHs = [
        loss._transform_kron([best_model[i].matrix(), best_model[(i + 1) % L].matrix()], original=True).detach().numpy()
        for i in range(L)
    ]
