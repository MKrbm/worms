...
struct options {
  ...
  :options(argv2vector(argc, argv), L_def, dim_def, T_def, M_def, false){}

  options(
    std::vector<std::string> argv, unsigned int L_def,
    unsigned int dim_def, double T_def, 
    std::string M_def, bool from_txt = true)
