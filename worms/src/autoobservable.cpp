...
namespace model
{
  BaseWormObs::BaseWormObs(size_t spin_dof, size_t legs): _spin_dof(spin_dof), _leg_size(legs) {
    if (legs != 1 && legs != 2) {std::cerr << "leg size of BaseWormObs must be 1 or 2" << std::endl; exit(1);}
  }

  int BaseWormObs::GetState(std::array<size_t, 4> spins) const  {
  ...
  bool BaseWormObs::check_is_symm() const {
    ...
    if (is_one_site()){
      return true;
    } else { // 2 points operator
      for (size_t _idx = 0; _idx < round(pow(_spin_dof, 2*2)); _idx++){
        size_t idx = _idx;
        size_t t_x = idx % _spin_dof; idx /= _spin_dof;
        size_t t_x_prime = idx % _spin_dof; idx /= _spin_dof;
        size_t h_x = idx % _spin_dof; idx /= _spin_dof;
  ...
  bool BaseWormObs::check_no_single() const {
    if (!has_operator()) { std::runtime_error("operator is not set yet"); }
    if (is_one_site()) { return true; }
    else {
      for (int _idx = 0; _idx < round(pow(_spin_dof, 2*3)); _idx++){
        size_t idx = _idx;
        size_t i = idx % _spin_dof; idx /= _spin_dof;
        size_t j = idx % _spin_dof; idx /= _spin_dof;
        size_t k = idx % _spin_dof; idx /= _spin_dof;
        if (operator()({i, j, i, k}) != 0 || operator()({j,i,k,i}) != 0 ) return false;
      }
    }
    return true;
  }
    ...
    if (is_one_site()) { return true; }
      ...
      for (size_t i = 0; i<_spin_dof; i++){
        for (size_t j = 0; j<_spin_dof; j++){
          if (operator()({i, j, i, j}) != 0) return false;
        }
      }
    }
    return true;
  ...
  */
  ...
  pair<BaseWormObsPtr, BaseWormObsPtr> WormObs::ReadNpy(size_t spin_dof , std::string folder_path){
    ...
    try{get_npy_path(folder_path, paths);}
