...
Worm<MCT>::Worm(double beta, MODEL model_, model::MapWormObs mp_worm_obs_,
    ...
    : spin_model(model_), L(spin_model.L), beta(beta), rho(-1),
      ...
      cstate(spin_model.L), cutoff_length(cl), _mp_worm_obs(mp_worm_obs_),
      _phys_cnt(1), loperators(spin_model.loperators),
      sps(spin_model.sps_sites(0)), obs_sum(mp_worm_obs_().size()),
      zw(spin_model.zw) {
...
template <class MCT> void Worm<MCT>::diagonalUpdate(double wdensity) {
    ...
    if (tau <
      ...
      if (r < pstart) {
        ...
        if (b < spin_model.Nb) {
          ...
          if (accept[u] > 1)
            throw std::runtime_error("accept[u] > 1");
          if (r < accept[u]) {
...
int Worm<MCT>::wormOpUpdate(int &next_dot, int &dir, int &site, double &wlength,
  ...
  if (fl == 0) {
    ...
    dout << "warp to : " << " next_dot = " << next_dot << " dotp_label = " << dotp->label()
         << "tau : " << opsp->tau() 
         << endl;
    if (opsp->op_type() >= 0) {
  ...
  if (dotp->at_origin()) {
    ...
    sign *=
        loperators[op_type]
            .signs[opsp->state()]; //! sign for single-flip is not included yet.
  ...
  warp_label:
      if (fl != 0) throw std::runtime_error("fl must be zero when warp_label is called");
      warped = true;
    ...
    if (tmp == 0) { // if head will warp.
      ...
      // it = std::begin(can_warp_ops);
      // std::advance(it, static_cast<size_t>(can_warp_ops.size() *
      // uniform(rand_src))); next_dot = *it; dotp = &spacetime_dots[next_dot];
      // opsp = &ops_main[dotp->label()];
      ...
      //   if (optype_ < 0){
      //     throw std::runtime_error("optype_ must be positive");
      //   }
      //   tmp = loperators[optype_].markov[state_num_](0, rand_src);
      //   dout << "in warp_label : " << " tmp : " << tmp << " state_num_ : " <<
      //   state_num_ << " optype_ : " << optype_ << endl; leg_size =
      //   opsp->size();
      // if (tmp == 0) // n* if tmp == 0
      // {
      ...
      // nindex = tmp / (sps - 1);       // sps_sites are filled with same
      // value. fl = tmp % (sps - 1) + 1; sign *=
      // _lop.signs[opsp->update_state(nindex, fl)]; // after procceeding dir =
      // nindex / (leg_size); site = opsp->bond(nindex % leg_size); if
      // (!_lop.has_warp(state_num_)) {
      ...
      //  t_x, t_x_prime); calcHorizontalGreen(tau, last_site, wt_site,
      ...
      dout << "non-warp" << "nindex : " << nindex << " fl : " << fl << " state_num : "
           << state_num << endl;
...
template <class MCT> void Worm<MCT>::reset_ops() {
  for (size_t i = 0; i < psop.size(); i++) {
    ops_main[psop[i]].set_state(pres[i]);
  }
}

template <class MCT> double Worm<MCT>::get_single_flip_elem(const OP_type &op) {
