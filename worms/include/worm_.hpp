...
class Worm{
  ...
  void wormUpdate(double& wcount, double& wlength){
    ...
    for (WORMS::iterator wsi = worms_list.begin(); wsi != worms_list.end(); ++wsi){
      ...
      do{
        ...
        size_t status = wormOpUpdate(n_dot_label, dir, site, wlength_prime, fl, tau, tau_prime, h_spin, h_spin_prime, w_label, w_site, w_tau);
      ...
      if (ini_dir != dir) {std::get<1>(*wsi) = (t_spin - fl) % sps_sites[site];}
      phys_cnt++;
  ...
  */
  void calcHorizontalGreen( double tau, size_t h_site, size_t t_site, 
    ...
    dout << " old :  " << h_x << " " << h_x_prime << " " << t_x << " " << t_x_prime << endl;
    // if (h_dir == t_dir){
    //   cout << "hi";
    // }
  ...
  void calcHorizontalGreen2( double tau, size_t h_site, size_t t_site, 
    ...
    // test specifically for HXXX.
    // calculate $\langle S^-_i S^+_j \rangle$
    if (h_x == 0 && h_x_prime == 1 && t_x == 1 && t_x_prime == 0) obs_sum2 ++; 
    if (h_x == 1 && h_x_prime == 0 && t_x == 0 && t_x_prime == 1) obs_sum2 ++; 
  }
  ...
  */
  int wormOpUpdate(size_t& next_dot, size_t& dir, 
    ...
    if (detectWormCross(tau, tau_prime, wt_tau, dir)){
      ...
      dout << " new : " <<  h_x << " " << h_x_prime << " " << t_x << " " << t_x_prime << endl;
      // if head is on tail, this case is not regarded as 2point correlation.
      if (site != wt_site) calcHorizontalGreen2(tau, site, wt_site, h_x, h_x_prime, t_x, t_x_prime);
    }
