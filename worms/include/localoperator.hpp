  ...
  markov_t &operator[](size_t s) {
    ...
    if (i < 0)
      throw std::invalid_argument(
          "probability that state s appears have to be 0");
    return markov[i];
  }
};

template <class MC> class local_operator {
private:
...
public:
  ...
  double ene_shift = 0; // energy shift to ensure that diagonal elements of
                        // hamiltonian are non-negative
  ...
  friend bool operator==
      <>(const local_operator<MC> &lhs, const local_operator<MC> &rhs);
};
// {
//   bool cmp = true;
//   cmp &= (lhs._ham == rhs._ham);
...
double alpha : parameter which determines how much the diagonal elements of
single flip will be allocated. alpha may be 1 / (1 + Nb) where Nb is the number
of NN bonds.
*/
template <class MC>
void local_operator<MC>::set_ham(double off_set, double thres, bool warp,
  ...
  if (leg > 2 && _ham_vector.size() > 1E5) {
    std::cout << "too big size of _ham_vector for non-bond operator : In the "
                 "case, only virtually one site operator at center is accepted"
              << std::endl;
    if (leg != 3)
      throw std::invalid_argument("leg more than 3 is not implemented yet");
    ...
    for (size_t s = 0; s < _ham_vector.size(); s++) {
      ...
      if (state.get_local_state(0) != state.get_local_state(3) &&
          state.get_local_state(2) != state.get_local_state(5))
        continue;
    ...
    for (size_t s = 0; s < _ham_vector.size(); s++) {
      ...
      std::vector<double> ogw = ogwt.init_table(_ham_vector, s, warp);
      if (warp) {
        for (int x = 1; x < ogw.size(); x++) {
          _has_warp[s] = _has_warp[s] || ((ogw[x] != 0) & (ogw[0] != 0));
        }
      }
      markov_tmp.push_back(markov_t(MC(), ogw));
    }
...
template <class MC> std::array<int, 2> local_operator<MC>::num2index(int num) {
