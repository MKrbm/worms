...
class Worm {
 ...
 public:
  ...
  Worm(double beta, MODEL model_, model::MapWormObs mp_worm_obs_,
       size_t cl = SIZE_MAX, int rank = 0, int seed = SEED);

  inline void initStates() {  //* initialized to all up
  ...
  //*append to worms
  inline void appendWorms(WORMS &wm, size_t site, size_t dot_label,
                          double tau) {
    wm.push_back(std::make_tuple(site, dot_label, tau));
  }

  inline bool is_same_state(int n, int m) { return n == m; }
  inline bool is_same_state(int n, state_t state, size_t lopt) {
    return n == state_funcs[lopt].state2num(state, state.size());
  }
  ...
  */
  void calcHorizontalGreen(double tau, size_t h_site, size_t t_site, size_t h_x,
                           size_t h_x_prime, size_t t_x, size_t t_x_prime,
  ...
  */
  ...
  // //*append to ops
  void appendOps(OPS &ops, DOTS &sp, std::unordered_set<size_t> &warp_sp,
                 const BOND *const bp, const BOND *const pp, int state,
                 int op_type, double tau);

  // //*append to ops
  void appendSingleOps(OPS &ops, DOTS &sp, std::unordered_set<size_t> &warp_sp,
                       int s_site, const BOND *const bp, const BOND *const pp,
  ...
  dir : direction of the worm moves to dot. 1 : upwards, 0 : downwards. So if
  dir = 1, it means worm comes from below the dot.
  */
  ...
  *perform one step from given Worm.
  ...
  ------
  */
  int wormOpUpdate(int &next_dot, int &dir, int &site, double &wlength, int &fl,
                   double &tau, const int wt_dot, const int wt_site,
                   const double wt_tau, int &w_x, int &wt_x, const int t_fl,
                   const int t_dir, const int w_index);
  /*
   *this function will be called after assigining op_main
   */
  void set_dots(size_t site, size_t dot_type, size_t index);
  void getSpinsDot(size_t next_dot, Dotv2 *dotp, int dir, size_t &h_x,
                   size_t &h_x_prime);
  ...
  */
  void checkOpsInUpdate(int worm_label, int p_label, int t_dir, int t_fl,
                        int fl, int dir);
  ...
  x_flipped : spin state after flipped. x // sps is upper spin and x % sps is
  lower spin. fl : if fl = 0, then worm doesn't change state.

  comments
  --------
  x_flipped wax originally x but it is changed to x_flipped after worm comes to
  the operator and flip either upper or lower spin.

  Original spin would be if direction is 1 (worm moves upwards) {x % sps + fl %
  sps,   x // sps}.
  */
