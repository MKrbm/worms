...
std::vector<double> exe_worm(model::base_model<MC> spin_model, double T,
                             size_t sweeps, size_t therms, size_t cutoff_l,
                             bool fix_wdensity) {
    ...
    for (int i = 0; i < therms + sweeps; i++) {
        ...
        if (cnt >= therms) {
            ...
            for (const auto &op : solver.ops_main) {
                int sign_ =
                    spin_model.loperators[op.op_type()].signs[op.state()];
                sign *= sign_;
                if (sign_ == -1)
                    n_neg++;
                n_op++;
                // w_rate *=
                // spin_model.loperators[op.op_type()].ham_rate_vector[op.state()];
                // cout <<
                // spin_model.loperators[op.op_type()].ham_rate_vector[op.state()]<<
                // endl;
            }
        ...
        if (i <= therms / 2) {
            if (!fix_wdensity) {
                if (wcount > 0)
                    wdensity =
                        std::max(spin_model.Nb / (wlength / wcount), (double)2);
                if (i % (therms / 8 + 1) == 0) {
                    wcount /= 2;
                    wlength /= 2;
                }
        ...
        if (i == therms / 2) {
            if (!fix_wdensity)
                std::cout << "Info: average number worms per MCS is reset from "
                          << spin_model.L << " to " << wdensity << "\n\n";
            else
                std::cout << "Info: average number worms per MCS is "
                          << wdensity << "\n\n";
        }
        cnt++;
    }
    ...
    return_value.push_back(std::sqrt(
        std::pow(ene.error(r) / ave_sign.mean(), 2) +
        std::pow(ene.mean() / std::pow(ave_sign.mean(), 2) * ave_sign.error(r),
                 2)));
    return_value.push_back(elapsed);
    return_value.push_back((therms + sweeps) / elapsed);

    std::cout << "Total Energy         = " << ene.mean() / ave_sign.mean()
              << " +- "
              << std::sqrt(std::pow(ene.error(r) / ave_sign.mean(), 2) +
                           std::pow(ene.mean() / std::pow(ave_sign.mean(), 2) *
                                        ave_sign.error(r),
                                    2))
              << std::endl;

    std::cout << "Elapsed time         = " << elapsed << " sec\n"
              << "Speed                = " << (therms + sweeps) / elapsed
              << " MCS/sec\n";
    std::cout << "Energy per site      = "
              ...
              << std::sqrt(std::pow(ene.error(r) / ave_sign.mean(), 2) +
                     ...
                     spin_model.L
              << std::endl
              << "average sign         = " << ave_sign.mean() << " +- "
              << ave_sign.error(r) << std::endl
              << "dimer operator       = " << sglt.mean() << std::endl
              << "# of operators       = " << n_ops.mean() << std::endl
              << "# of neg sign op     = " << n_neg_ele.mean() << std::endl
              << "breakout rate        = " << 1 - r << std::endl;
    return return_value;
}
...
exe_worm_parallel(model::base_model<MC> spin_model, double T, size_t sweeps,
    ...
    Worm<MC> solver(beta, spin_model, cutoff_l,
