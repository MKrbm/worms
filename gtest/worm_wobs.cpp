...
void run_worm(model::base_model<MC> &spin, double T, size_t sweeps,
  ...
  pair<double, double> worm_obs_mean =
      jackknife_reweight_div(worm_obs, phys_conf);  // calculate <WoS> / <S>
  ...
  cout << "temperature          = " << T << endl
       << "Average sign         = " << as_mean.first << " +- " << as_mean.second
       << endl;
  cout << "-------------------------------------------------" << endl;
  cout << "Energy per site      = " << ene_mean.first / ns << " +- "
       << ene_mean.second / ns << endl
       << "Specific heat        = " << c_mean.first / ns << " +- "
       << c_mean.second / ns << endl
       << "magnetization        = " << m_mean.first * T / ns << " +- "
       << m_mean.second * T / ns << endl
       << "susceptibility       = " << chi_mean.first * T / ns << " +- "
       << chi_mean.second * T / ns << endl
       << "G                    = " << worm_obs_mean.first << " +- "
       << worm_obs_mean.second << endl
       << "worm obs mean        = " << worm_obs.mean()[0] << " +- "
       << worm_obs.mean()[1] << endl
       << "Physical configurations = " << phys_conf.mean()[0] << " +- "
       << phys_conf.mean()[1] << endl;
  return;
}
...
TEST(WormSimuObs, HXXX1D_NS) {
  ...
  ham_path =
      ...
      "Jz_-1_Jx_0.5_Jy_0.3_hz_0_hx_0.5/H";
  obs_path =
      "../gtest/model_array/Heisenberg/1D/original/"
      "Jz_-1_Jx_0.5_Jy_0.3_hz_0_hx_0.5/Sz";
  wobs_path = "../gtest/model_array/worm_obs/g_test";
  model::base_model<MC> spin(lat, dofs, ham_path, params, types, shift, zw,
                             false, false);
...
TEST(WormSimuObs, HXXX1D_2S_NS) {
  ...
  // This hamiltonian requires zero worm.
  ham_path =
      "../gtest/model_array/Heisenberg/1D/2sites/"
      "Jz_-1_Jx_0.5_Jy_0.3_hz_0_hx_0.5/H";
  obs_path =
      "../gtest/model_array/Heisenberg/1D/2sites/"
      "Jz_-1_Jx_0.5_Jy_0.3_hz_0_hx_0.5/Sz";
...
TEST(WormSimuObs, HXXX2D_0) {
  ...
  ham_path =
      "../gtest/model_array/Heisenberg/2D/original/"
      "Jz_-1_Jx_0.5_Jy_0.3_hz_0_hx_0.5/H";
  obs_path =
      "../gtest/model_array/Heisenberg/2D/original/"
      "Jz_-1_Jx_0.5_Jy_0.3_hz_0_hx_0.5/Sz";
